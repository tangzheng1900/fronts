   ...
   // clearArea('definition');
    ...
    request.onreadystatechange = function () {
        if (request.readyState === 4) {
            ...
            alert(""+word+"："+response.voc.definition);
            if (response.voc != '')
                //queryOk(response);
                {
...
chrome.extension.onMessage.addListener(
    function(request, sender, sendResponse) {
        ...
        var result=query(request.greeting);
        if(result.definition!=''&&result.definition!=undefined){
            sendResponse(result);
            result={};

        }

        else{sendResponse({definition: "goodbye"});}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelector('#excute').addEventListener('click',exctejs);
});
